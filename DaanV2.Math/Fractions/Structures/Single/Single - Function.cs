...
namespace DaanV2.Math.Fractions {
    public readonly partial struct SingleFraction {
        ...
        /// <returns>An <see cref="SingleFraction"/></returns>
        public static SingleFraction Parse(String s) {
            return Parse(s.AsSpan());
        }

        /// <summary>Converts the span representation of a number to its Single-precision floating-point number equivalent.</summary>
        /// <param name="s">The span to parse</param>
        /// <returns>An <see cref="SingleFraction"/></returns>
        public static SingleFraction Parse(ReadOnlySpan<Char> s) {
            ...
            return new SingleFraction(Single.Parse(num), Single.Parse(den));
        }
    }
}
